    ...
    private void encryptConfig(Connection conn) {
        ...
        try {
            ...
            while (rs.next()) {
                ...
                try (PreparedStatement update_configuration_pstmt = conn.prepareStatement("update `cloud`.`configuration` set value=?, category = 'Hidden' where name=?");) {
                    update_configuration_pstmt.setBytes(1, encryptedValue.getBytes("UTF-8"));
                    update_configuration_pstmt.setString(2, name);
                    update_configuration_pstmt.executeUpdate();
                } catch (SQLException e){
                    throw new CloudRuntimeException("UpdateConfiguration:Exception" + e.getMessage(), e);
                }
            }
        } catch (SQLException e) {
    ...
    private void encryptClusterDetails(Connection conn) {
        ...
        try {
            ...
            while (rs.next()) {
                ...
                String encryptedValue = DBEncryptionUtil.encrypt(value);
                try (PreparedStatement update_cluster_details_pstmt = conn.prepareStatement("update `cloud`.`cluster_details` set value=? where id=?");) {
                    update_cluster_details_pstmt.setBytes(1, encryptedValue.getBytes("UTF-8"));
                    update_cluster_details_pstmt.setLong(2, id);
                    update_cluster_details_pstmt.executeUpdate();
                } catch (SQLException e) {
                    throw new CloudRuntimeException("Unable encrypt cluster_details values: Exception " + e.getMessage(), e);
                }
            }
        } catch (SQLException e) {
