...
public class Upgrade410to420 implements DbUpgrade {
    ...
    private void removeFirewallServiceFromSharedNetworkOfferingWithSGService(Connection conn) {
        ...
        try {
            ...
            while (rs.next()) {
                ...
                try (PreparedStatement update_ntwk_offering_service_map_pstmt = conn.prepareStatement("DELETE from `cloud`.`ntwk_offering_service_map` where network_offering_id=? and service='Firewall'");) {
                    update_ntwk_offering_service_map_pstmt.setLong(1, id);
                    update_ntwk_offering_service_map_pstmt.executeUpdate();
                } catch (SQLException e) {
                    throw new CloudRuntimeException("Unable to remove Firewall service for SG shared network offering:Exception " + e.getMessage(), e);
                }
            }
        } catch (SQLException e) {
        ...
        try {
            ...
            while (rs.next()) {
                ...
                if (index > 1) {
                    ...
                    try (PreparedStatement update_snapshots_pstmt = conn.prepareStatement("UPDATE `cloud`.`snapshots` set backup_snap_id=? where id = ?");) {
                        update_snapshots_pstmt.setString(1, correctedPath);
                        update_snapshots_pstmt.setLong(2, id);
                        update_snapshots_pstmt.executeUpdate();
                    } catch (SQLException e) {
                        throw new CloudRuntimeException("Unable to update backup id for KVM snapshots:Exception " + e.getMessage(), e);
                    }
                }
            }
    ...
    private void migrateSnapshotStoreRef(Connection conn) {
        ...
        PreparedStatement snapshotUpdate = null;
        PreparedStatement snapshotWithNoBkpId = null;
        PreparedStatement snapshotRemapParent = null;
        ResultSet rs = null;

        s_logger.debug("Updating snapshot_store_ref table from snapshots table");
        try {
            snapshotWithNoBkpId = conn.prepareStatement("SELECT s.id FROM `cloud`.`snapshots` s where s.backup_snap_id is null and exists (select 1 from `cloud`.`snapshots` s1 where s1.prev_snap_id = s.id ) order by id");
            rs = snapshotWithNoBkpId.executeQuery();

            while(rs.next()){
                Long snapshotId = rs.getLong("id");
                s_logger.debug("Remap prev_snap_id of all snapshot with id " + Long.toString(snapshotId));
                snapshotRemapParent = conn.prepareStatement("UPDATE `cloud`.`snapshots` s ,`cloud`.`snapshots` s1  set s.prev_snap_id = s1.prev_snap_id  where s.prev_snap_id = ? and  s1.id = ?");
                snapshotRemapParent.setLong(1,snapshotId);
                snapshotRemapParent.setLong(2,snapshotId);
                int rowCount = snapshotRemapParent.executeUpdate();
                snapshotRemapParent.close();
                s_logger.debug("Re-map Parent, Update modified " + rowCount + " rows");
            }

            rs.close();
            snapshotWithNoBkpId.close();
            //Update all snapshots except KVM snapshots
            snapshotStoreInsert =
                    conn.prepareStatement("INSERT INTO `cloud`.`snapshot_store_ref` (store_id,  snapshot_id, created, size, parent_snapshot_id, install_path, volume_id, update_count, ref_cnt, store_role, state) select sechost_id, id, created, size, prev_snap_id, CONCAT('snapshots', '/', account_id, '/', volume_id, '/', backup_snap_id), volume_id, 0, 0, 'Image', 'Ready' from `cloud`.`snapshots` where status = 'BackedUp' and hypervisor_type <> 'KVM' and sechost_id is not null and backup_snap_id is not null");
            ...
            snapshotUpdate= conn.prepareStatement("UPDATE `cloud`.`snapshots` set removed = null, status = 'Destroyed' where removed is not null and sechost_id is not null and backup_snap_id is not null");
            rowCount = snapshotUpdate.executeUpdate();
            ...
            try {
                ...
                if(snapshotWithNoBkpId != null){
                    snapshotWithNoBkpId.close();
                }
                if(snapshotRemapParent != null){
                    snapshotRemapParent.close();
                }
                if(rs != null){
                    rs.close();
    ...
    private void encryptSite2SitePSK(Connection conn) {
        ...
        try {
            ...
            while (rs.next()) {
                ...
                try (PreparedStatement update_s2s_customer_gateway_pstmt = conn.prepareStatement("update `cloud`.`s2s_customer_gateway` set ipsec_psk=? where id=?");) {
                    update_s2s_customer_gateway_pstmt.setBytes(1, encryptedValue.getBytes("UTF-8"));
                    update_s2s_customer_gateway_pstmt.setLong(2, id);
                    update_s2s_customer_gateway_pstmt.executeUpdate();
                } catch (SQLException e) {
                    throw new CloudRuntimeException("Unable to encrypt Site2Site Customer Gateway pre-shared key : Exception " + e.getMessage(), e);
                }
            }
        } catch (SQLException e) {
    ...
    private void migrateDatafromIsoIdInVolumesTable(Connection conn) {
        ...
        try {
            pstmt = conn.prepareStatement("select * from information_schema.columns where table_schema='cloud' and table_name='volumes' and column_name='iso_id1'");
            rs = pstmt.executeQuery();
            if (rs.next()) {
                pstmt.close();
            ...
            s_logger.debug("Failed to execute migrateDatafromIsoIdInVolumesTable "+e);
        } finally {
            try {
                if (pstmt != null) {
                    pstmt.close();
