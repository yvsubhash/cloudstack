...
var g_logger;
// Below variables are meant for browser detection
...
KeyboardMapper.prototype = {
    ...
    isConditionalParametersMatched : function(entry, guestos, browser, browserVersion, guestosDisplayName, hypervisor, hypervisorVersion) {
            ...
            if(entry.browser != undefined) {
                if(!((isChrome && entry.browser.toLowerCase() == 'chrome') || (isSafari && entry.browser.toLowerCase() == 'safari') ||
                    (isFirefox && entry.browser.toLowerCase() == 'firefox') || (isOpera && entry.browser.toLowerCase() == 'opera') ||
                    (isIE && entry.browser.toLowerCase() == 'ie'))) {
                        return false;
                }
            }
    ...
    isConditionalEntryMatched : function(eventType, code, modifiers, entry, guestos, browser, browserVersion, guestosDisplayName, hypervisor, hypervisorVersion) {
        ...
        return this.isConditionalParametersMatched(entry, guestos, browser, browserVersion, guestosDisplayName, hypervisor, hypervisorVersion);
    },
...
// ajaxKeys.js
    ...
    setupKeyboardTranslationTable : function() {
        ...
        this.keyboardMapper.setKeyboardType(this.currentKeyboard);
        ...
        if (keyboardTables != undefined ) {
            ...
            for(var i = 0; i < keyboardTables.length; i++) {
                ...
                if(this.currentKeyboard != mappingTbl.keyboardType) {
                    continue;
                }
                ...
                for (var j = 0; j < x11Maps.length; j++) {
                    ...
                    if(this.keyboardMapper.isConditionalParametersMatched(x11Maps[j], this.guestos, $.browser, $.browserVersion, this.guestosDisplayName,
                     this.hypervisor, this.hypervisorVersion)) {
                        this.keyboardMapper.jsX11KeysymMap[code] = mappedEntry;
                    }
                }
                ...
                for (var j = 0; j < keyPressMaps.length; j++) {
                    ...
                    if(this.keyboardMapper.isConditionalParametersMatched(keyPressMaps[j], this.guestos, $.browser, $.browserVersion, this.guestosDisplayName,
                                         this.hypervisor, this.hypervisorVersion)) {
                        this.keyboardMapper.jsKeyPressX11KeysymMap[code] = mappedEntry;
                    }
                }
            }
        }
