...
public class VmwareStorageManagerImpl implements VmwareStorageManager {
    ...
    public boolean execute(VmwareHostService hostService, CreateEntityDownloadURLCommand cmd) {
        ...
        int timeout = NumbersUtil.parseInt(cmd.getContextParam(VmwareManager.s_vmwareOVAPackageTimeout.key()), 3600000);
        if (data == null) {
            return false;
        }
        ...
        try {
            ...
            if (cmd.toSecondaryStorage()) {
                ...
                if (morDatastore == null) {
                    morDatastore = hyperHost.mountDatastore(poolTO.getType(), poolTO.getHost(), 0, poolTO.getPath(), poolTO.getUuid().replace("-", ""));
