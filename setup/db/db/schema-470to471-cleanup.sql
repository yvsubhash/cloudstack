...
DELETE `cloud`.`vm_network_map` FROM `cloud`.`vm_network_map` join `cloud`.`vm_instance` on `vm_network_map`.`vm_id` = `vm_instance`.`id` where `vm_instance`.`removed` is not null;
